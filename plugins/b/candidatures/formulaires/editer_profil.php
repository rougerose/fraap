...
function formulaires_editer_profil_verifier_dist($id_auteur='new', $retour='', $lier_id_article=0, $config_fonc='auteurs_edit_config', $row=array(), $hidden=''){
	$erreurs = formulaires_editer_objet_verifier('auteur',$id_auteur,array('nom','prenom','activite'));

	// repris de prive/formulaires/editer_auteur.php
...
function formulaires_editer_profil_traiter_dist($id_auteur='new', $retour='', $lier_id_article=0, $config_fonc='auteurs_edit_config', $row=array(), $hidden=''){
	...
	$ok = formulaires_editer_objet_traiter('auteur',$id_auteur,0,0,$retour,$config_fonc,$row,$hidden);

	if ($ok == true) {
		return array(
			'message_ok' => _T('stages:message_ok'),
			'id_auteur' => $id_auteur,
			'editable' => true
		);
	} else {
		return array(
			'message_erreur' => _T('stages:message_erreur'),
			'id_auteur' => $id_auteur,
			'editable' => true
		);
	}
